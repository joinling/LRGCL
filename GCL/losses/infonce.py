...
class InfoNCE(Loss):
    ...
    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        sim = _similarity(anchor, sample) / self.tau # 1024,2048
        exp_sim = torch.exp(sim) * (pos_mask + neg_mask) # 1024,2048
        log_prob = sim - torch.log(exp_sim.sum(dim=1, keepdim=True)) # 1024,2048
        loss = log_prob * pos_mask # 1024,
        loss = loss.sum(dim=1) / pos_mask.sum(dim=1) # 1024,
        return -loss.mean()


class InfoNCE_1(Loss):
    def __init__(self, tau):
        super(InfoNCE_1, self).__init__()
        self.tau = tau

    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        sim = _similarity(anchor, sample) / self.tau # 1024,2048
        sima = sim * (pos_mask + neg_mask)
        exp_sim = torch.exp(sima) * (pos_mask + neg_mask) # 1024,2048
        log_prob = sima - torch.log(exp_sim.sum(dim=1, keepdim=True)) # 1024,2048
        # loss = log_prob * (pos_mask + neg_mask)  # 1024,
        loss = log_prob * pos_mask # 1024,
        loss = loss.sum(dim=1) / pos_mask.sum(dim=1) # 1024,
...
class InfoNCEweight(Loss):
    def __init__(self, tau):
        super(InfoNCEweight, self).__init__()
        self.tau = tau

    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        sim = _similarity(anchor, sample) * _similarity(anchor, sample) / self.tau
        # sim = sim * sim
