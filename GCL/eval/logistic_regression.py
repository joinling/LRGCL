...
class LREvaluator_Binary(BaseEvaluator):
    def __init__(self, num_epochs: int = 5000, learning_rate: float = 0.01,
                 weight_decay: float = 0.0, test_interval: int = 20):
        self.num_epochs = num_epochs
        self.learning_rate = learning_rate
        ...
        with tqdm(total=self.num_epochs, desc='(LR)',
            ...
            for epoch in range(self.num_epochs):
                ...
                if (epoch + 1) % self.test_interval == 0:
                    ...
                    y_val = y[split['valid']].detach().cpu().numpy()
                    y_pred = classifier(x[split['valid']]).argmax(-1).detach().cpu().numpy()
                    val_weighted = f1_score(y_val, y_pred, average='weighted')
                    ...
                    if val_auc > best_val_auc:
                        best_val_micro = val_weighted
                        # best_test_micro = test_micro
                        best_test_auc = test_auc
                        # best_epoch = epoch

                    pbar.set_postfix({'test F1WE': test_weighted
...
class LREvaluator_Binary_noval(BaseEvaluator):
    def __init__(self, num_epochs: int = 5000, learning_rate: float = 0.01,
                 weight_decay: float = 0.0, test_interval: int = 20):
        self.num_epochs = num_epochs
        self.learning_rate = learning_rate
...
class LREvaluator_Binary4GRACE(BaseEvaluator):
    def __init__(self, num_epochs: int = 5000, learning_rate: float = 0.01,
                 weight_decay: float = 0.0, test_interval: int = 20):
        self.num_epochs = num_epochs
        self.learning_rate = learning_rate
