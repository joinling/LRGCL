...
class HMLC(nn.Module):
    def __init__(self, temperature=0.07,
        ...
        self.sup_con_loss = DualBranchContrast()
        self.loss_type = loss_type

    def pow_2(self, value):
        return torch.pow(2, value)

    def forward(self, h1, h2, labels):

        mask = torch.ones(labels.shape)  # N*L
        ...
        for l in range(1, labels.shape[1]):
            ...
            layer_labels = labels * mask  # N*L, 按元素*
            # .all =>(N,) 当前label level下的标签相同情况
            # stack所有image在当前label level下的标签
            ...
            layer_loss = self.sup_con_loss(h1=h1, h2=h2, extra_pos_mask=mask_labels)
            ...
            # # layer_labels相同，就默认是该label level下的同一个商品，不进行重复计算
