    ...
    for e in range(args.epoch):
        ...
        for (input_nodes1, output_nodes1, blocks1), (input_nodes2, output_node2s, blocks2) in zip(train_loader1, train_loader2):
            ...
            if args.linked_pos ==True:
                ...
                # rela_mask = torch.cat((rela_mask
                #                         , torch.from_numpy((matrix > 1).astype(bool)).unsqueeze(2)), dim=2)
                # # rela_mask :[bsz, bsz, 4] 不同标签->3种关系->2种->1种
                # rela_mask = torch.cat((rela_mask, torch.from_numpy(matrix.astype(bool)).unsqueeze(2)), dim=2)
                ...
                for l in range(0, rela_mask.shape[2]):
                    ...
                    layer_mask_pos = extra_pos_mask * rela_mask[:, :, l]  # [bsz, bsz]
                    layer_mask_pos = torch.cat([layer_mask_pos, layer_mask_pos], dim=1)
                    layer_mask_pos.fill_diagonal_(False)
                    # print('layer_mask type', type(layer_mask))
                    extra_neg_mask = torch.ne(blocks1[-1].dstdata['label'][category],
                                              blocks1[-1].dstdata['label'][category].unsqueeze(dim=1))
                    layer_mask_neg = extra_neg_mask * rela_mask[:, :, l]
                    layer_mask_neg = torch.cat([layer_mask_neg, layer_mask_neg], dim=1)
                    layer_mask_neg.fill_diagonal_(False)
                    # layer_mask_neg = torch.cat([layer_mask_neg, layer_mask_neg], dim=1)
                    # Sup_Con loss
                    layer_loss = contrast_model(h1=h1, h2=h2, extra_pos_mask=layer_mask_pos, extra_neg_mask=layer_mask_neg)
                    layer_loss = torch.max(max_loss_lower_layer, layer_loss)
                    cumulative_loss += torch.pow(3, torch.tensor(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta', type=float, default=0.001, help='the weight of Contrastive Loss, refer to rumor detection')
    ...
    if args.dataset == 'yelp':
        graph, in_feats, n_nodes, test_nid_dict, train_nid_dict = dgl_get_yelp_hetero(args.dtrain_ratio)
