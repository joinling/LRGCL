...
from yelp_aug_v5 import dgl_get_amz_hetero, dgl_get_yelp_hetero, dgl_get_yelp_clean, dgl_get_yelp_hetero_aug, dgl_get_amz_hetero_aug
...
def train(model, g, g1, g2, args, i):
    ...
    smapler = MultiLayerNeighborSampler([8]*2)
    ...
    for e in range(args.epoch):
        ...
        for (input_nodes1, output_nodes1, blocks1), (input_nodes2, output_node2s, blocks2) in zip(train_loader1, train_loader2):
            ...
            # Sup_Con loss
            loss1 = contrast_model(h1=h1, h2=h2, extra_pos_mask=extra_pos_mask, extra_neg_mask=extra_neg_mask)
            loss2 = contrast_model(h1=h1, h2=h2, extra_pos_mask=None, extra_neg_mask=None)
            ...
            if args.loss == 'suploss':
                total_loss = CEloss + args.beta * loss1
            else:
                total_loss = CEloss + args.beta * loss1 + args.beta * loss2
            ################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feat_mask_prob', type=float, default=0.0)
    parser.add_argument('--beta', type=float, default=0.1, help='the weight of Contrastive Loss, refer to rumor detection')
    parser.add_argument('--temp', type=float, default=0.2, help='Temperature.')
    parser.add_argument("--loss", type=str, default="suploss",
                        help="suploss, multiloss")
    parser.add_argument("--cat", type=int, default=0, help="1 for cat initial feature and 0 for do not")
    ...
    for i in range(0, args.runtime, 1):
        ...
        if args.dataset == 'yelp':
            ...
            g2 = dgl_get_yelp_clean(args.ddataset, 0)
            category = 'review'
            unrelevent1 = 'item'
            unrelevent2 = 'user'
        elif args.dataset == 'amazon':
