        ...
        for (input_nodes1, output_nodes1, blocks1), (input_nodes2, output_node2s, blocks2) in zip(train_loader1, train_loader2):
            ...
            if args.linked_pos ==True:
                ...
                for l in range(0, rela_mask.shape[2]):
                    ...
                    layer_mask = pos_label_mask * rela_mask[:, :, l]  # [bsz, bsz]
                    ...
                    # Sup_Con loss
                    layer_loss = contrast_model(h1=h1, h2=h2, extra_pos_mask=layer_mask, extra_neg_mask=extra_neg_mask)
    ...
    if args.run == 1:
        if homo:
            ...
            if args.model == 'RGAT':
                ...
                model = StochasticTwoLayerRGCN(in_feats, h_feats, out_feat=64, rel_names=graph.etypes, feat_dim=in_feats)
