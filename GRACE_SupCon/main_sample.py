...
def train(model, g, g1, g2, args):
    ...
    idx_train, idx_rest, y_train, y_rest = train_test_split(index, labels[index], stratify=labels[index],
                                                            ...
                                                            random_state=args.seed1, shuffle=True)
    idx_valid, idx_test, y_valid, y_test = train_test_split(idx_rest, y_rest, stratify=y_rest,
                                                            test_size=0.67,
                                                            random_state=args.seed2, shuffle=True)
    ...
    for e in range(args.epoch):
        ...
        tmf1 = f1_score(labels[test_mask], preds[test_mask], average='micro')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--seed1", type=int, default=1,
                        help="")
    parser.add_argument("--seed2", type=int, default=2,
                        help="")
    ...
    if args.run == 1:
        if homo:
            ...
            if args.model == 'RGAT':
                ...
                model = StochasticTwoLayerRGCN(in_feats, h_feats,feat_dim=32, out_feat=64, rel_names=graph.etypes)
                # model = RGCN(in_feats, h_feats, num_classes, len(graph.canonical_etypes))

        train(model, graph, g1, g2, args)

"""
王鲁老师建议：
对于recall较低的问题，
在开始训练之前，虚拟出若干true的节点，减少不平衡
虚拟方法，参考smote eg.(node1+node2)/2 --> node k
新增的节点不添加结构信息
"""
