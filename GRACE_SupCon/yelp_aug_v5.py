...
dgl_get_yelp_hetero(1, 0.2)

def dgl_get_yelp_clean(dataset, train_ratio):
    if dataset == 'YelpChi':
        yelp = scio.loadmat('YelpChi.mat')
    else:
        yelp = scio.loadmat('YelpNYC.mat')
    ...
    rur_label_src = label_tensor[rur_tensor_src]
    rur_label_dis = label_tensor[rur_tensor_dis]
    rtr_label_src = label_tensor[rtr_tensor_src]
    rtr_label_dis = label_tensor[rtr_tensor_dis]
    rsr_label_src = label_tensor[rsr_tensor_src]
    rsr_label_dis = label_tensor[rsr_tensor_dis]
    rur = torch.eq(rur_label_src, rur_label_dis)+0
    rtr = torch.eq(rtr_label_src, rtr_label_dis)+0
    rsr = torch.eq(rsr_label_src, rsr_label_dis)+0
    rur_clean = torch.squeeze(torch.nonzero(rur))
    rtr_clean = torch.squeeze(torch.nonzero(rtr))
    rsr_clean = torch.squeeze(torch.nonzero(rsr))

    graph_data = {
        ('review', 'user', 'review'): (rur_tensor_src[rur_clean], rur_tensor_dis[rur_clean]),
        ('review', 'time', 'review'): (rtr_tensor_src[rtr_clean], rtr_tensor_dis[rtr_clean]),
        ('review', 'star', 'review'): (rsr_tensor_src[rsr_clean], rsr_tensor_dis[rsr_clean]),
        ('user', 'dislike', 'item'): (dislike_src, dislike_dst)  # no-use
    }
...
# g, n_hetero_feat, n_nodes = dgl_get_yelp_clean('YelpChi', 0.2)
...
def dgl_get_yelp_hetero_aug(dataset, edge_mask_prob, feat_mask_prob):
    ...
    g = g.add_self_loop(etype='user')
    g = g.add_self_loop(etype='time')
    g = g.add_self_loop(etype='star')
...
def dgl_get_amz_hetero_aug(edge_mask_prob, feat_mask_prob):
    ...
    g = g.add_self_loop(etype='p')
    g = g.add_self_loop(etype='s')
    g = g.add_self_loop(etype='v')
